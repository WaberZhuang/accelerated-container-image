...
func init() {
	...
	rootCmd.Flags().BoolVar(&referrer, "referrer", false, "push converted manifests with subject, note `--oci` is forced true if `--referrer` is set, cause the referrer must be in OCI format.")
