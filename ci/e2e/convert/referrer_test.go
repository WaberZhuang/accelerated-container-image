...
const (
	ArtifactTypeOverlaybd = "application/vnd.alibaba.overlaybd.native.v1+json"
	ArtifactTypeTurboOCI  = "application/vnd.alibaba.overlaybd.turbo.v1+json"
)
...
func TestConvertReferrer(t *testing.T) {
	...
	repo, err := remote.NewRepository("localhost:5000/hello-world")
	require.Nil(err, err)
	...
	checkReferrer = func(src, dst ocispec.Descriptor, artifactType string) {
		...
		case ocispec.MediaTypeImageManifest, dockerspec.MediaTypeDockerSchema2Manifest:
			...
			require.Equal(src, *manifest.Subject)
		case ocispec.MediaTypeImageIndex, dockerspec.MediaTypeDockerSchema2ManifestList:
			...
			require.Equal(src, *index.Subject)
	...
	testcases := []struct {
		...
		{
			name:      "simple",
			reference: "localhost:5000/hello-world:amd64",
		},
		{
			name:      "index",
			reference: "localhost:5000/hello-world:multi-arch",
		},
		{
			name:      "nested index",
			reference: "localhost:5000/hello-world:nested-index",
		},
	}
...
func copyImageToRegistry(ctx context.Context) error {
	...
	for _, img := range images {
		src := img
		dst := fmt.Sprintf("localhost:5000/hello-world:%s", img)
